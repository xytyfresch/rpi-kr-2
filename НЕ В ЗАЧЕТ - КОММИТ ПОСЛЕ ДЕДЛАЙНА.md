Сейчас при добавлении не проставляется id-шник. Из-за этого некорректно работает удаление новых записей.

Фиксится так: 
1. В конструкторе получаем текущий id
```js
    this.currentId = this.transactions.length;
```
2. При каждом добавлении увеличиваем его на 1
```js
    newTransaction.id = ++this.currentId;
```

Исправленная версия файла /transaction-list-presenter.js:
```js
